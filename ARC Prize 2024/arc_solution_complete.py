#!/usr/bin/env python3
"""
ARC Prize 2024 Solution - Complete Script
...
#arc_train_set = ArcDataset.from_file('./arc-prize-2024/arc-agi_training_challenges.json')
...
# 根据环境选择模型加载模式
if HAS_CUDA:
    MODEL_MODE = 'transformers_bf16_4bit'  # 有 GPU 时使用量化
else:
    MODEL_MODE = 'transformers'  # macOS/CPU 使用标准 transformers

# 模型配置
# 选项1: 使用本地模型路径（如果已下载）
# base_model = '/path/to/local/model'

# 选项2: 使用 HuggingFace 模型名称（需要 token 如果是私有仓库）
base_model = 'wb55l_nemomini_fulleval'  # 原始模型

# 选项3: 使用替代的公开模型（如果原始模型不可用）
# base_model = 'microsoft/phi-2'  # 示例：使用公开的小模型进行测试

MyFormatter, perm_aug, max_seq_length_train, mask_first = ArcFormatter_premix_3, 'rnd_all', 4224, 0

# training & inference
train_epochs = 4
multi_gpu_train = HAS_CUDA  # 只在有 CUDA 时启用多 GPU
...
def prepare_run(model_path, load_lora=None, train=False, gpu=None, **kwargs):
    # macOS 环境：不使用 GPU 配置
    if gpu is not None and HAS_CUDA:
        os.environ["CUDA_DEVICE_ORDER"   ] = "PCI_BUS_ID"
        os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu)
    elif not HAS_CUDA:
        # macOS/CPU 模式：确保使用 CPU
        os.environ.pop("CUDA_VISIBLE_DEVICES", None)

    # 检查模型路径：如果是本地路径则使用 local_files_only=True
    # 判断是否为本地路径：
    # 1. 路径存在 -> 本地路径
    # 2. 以 './' 或 '../' 开头 -> 本地路径
    # 3. 是绝对路径 -> 本地路径
    # 4. 包含多个 '/' 且不是 HuggingFace 格式 (username/repo) -> 本地路径
    is_local_path = False
    if isinstance(model_path, str):
        if os.path.exists(model_path):
            ...
            # 包含 '/'，可能是 HuggingFace repo ID 或本地路径
            parts = model_path.split('/')
            if len(parts) == 2 and not os.path.exists(model_path):
                # 格式为 username/repo 且路径不存在 -> HuggingFace repo ID
                is_local_path = False
            else:
                # 其他情况视为本地路径
                is_local_path = True
        else:
            # 单个名称，可能是 HuggingFace repo 或本地路径
            # 如果路径不存在，假设是 HuggingFace repo
            is_local_path = os.path.exists(model_path)
    
    local_files_only = is_local_path
    
    # 如果使用 HuggingFace token，从环境变量获取
    ...
    if is_local_path and not os.path.exists(model_path):
        raise FileNotFoundError(
            ...
            f"请确保训练已完成并保存了模型，或使用正确的模型路径"
        )
    
    model, tokenizer, formatter = prepare_model(  # base model configuration
        model=model_path,
        local_files_only=local_files_only,  # 本地路径时只使用本地文件
        mode=MODEL_MODE,  # 使用根据环境选择的模式
...
def start_training(gpu):
    try:
        # 在 CPU 模式下，gpu 可能是 None，需要转换为 0
        gpu_id = gpu if gpu is not None else 0
        storage_path = f'{model_temp_storage}_gpu{gpu_id if (multi_gpu_train and HAS_CUDA) else 0}'
        
        # 训练条件：
        # 1. GPU 模式：gpu == 0 或启用多 GPU
        # 2. CPU 模式：总是训练（gpu 为 None 或 0）
        should_train = (gpu_id == 0) or (multi_gpu_train and HAS_CUDA) or (not HAS_CUDA)
        
        if not should_train:
            print(f'*** GPU {gpu_id}: 跳过训练（多 GPU 模式下仅在 GPU 0 训练）')
            os.makedirs(f'{storage_path}_done', exist_ok=True)
            return
        
        if os.path.exists(storage_path):
            print(f'*** 模型已存在: {storage_path}，跳过训练')
            os.makedirs(f'{storage_path}_done', exist_ok=True)
            return
        
        print(f'*** 开始训练，模型将保存到: {storage_path}')
        with RemapCudaOOM():
            model, formatter = prepare_run(base_model, train=True, gpu=gpu_id if HAS_CUDA else None)
            dataset = prepare_dataset(formatter, train=True, gpu=gpu_id if (multi_gpu_train and HAS_CUDA) else None)
            # macOS/CPU 环境：减小批次大小
            batch_size = 1 if not HAS_CUDA else 2
            grad_acc = 4 if not HAS_CUDA else 2
            optim_name = "adamw_torch" if not HAS_CUDA else "adamw_8bit"
...
def start_inference(gpu):
    ...
    # 检查训练是否完成
    done_signal = f'{storage_path}_done'
    if not os.path.exists(done_signal):
        print(f'*** 等待训练完成... (检查 {done_signal})')
        max_wait = 3600  # 最多等待1小时
        waited = 0
        while not os.path.exists(done_signal) and waited < max_wait:
            time.sleep(15)
            waited += 15
            if waited % 60 == 0:
        ...
        if not os.path.exists(done_signal):
            print(f'*** 错误: 训练未完成或超时 (等待 {max_wait} 秒)')
            print(f'*** 请检查训练是否正常运行')
            return
    
    # 检查模型路径是否存在
    if not os.path.exists(storage_path):
        print(f'*** 错误: 模型路径不存在: {storage_path}')
        print(f'*** 请确保训练已完成并保存了模型')
        return
    
    with RemapCudaOOM():
        model, formatter = prepare_run(storage_path, gpu=gpu_id if HAS_CUDA else None)
        dataset = prepare_dataset(formatter, train=False, gpu=gpu_id if (multi_gpu_train and HAS_CUDA) else None)
        # macOS/CPU 环境：调整参数
        batch_size = 1 if not HAS_CUDA else 2
        grad_acc = 4 if not HAS_CUDA else 2
        optim_name = "adamw_torch" if not HAS_CUDA else "adamw_8bit"
...
def main():
    parser = argparse.ArgumentParser(description='ARC Prize 2024 Solution - Local Runner')
    parser.add_argument('--mode', choices=['train', 'inference', 'full'], default='full',
                       help='运行模式: train=仅训练, inference=仅推理, full=训练+推理')
    parser.add_argument('--gpu', type=int, default=0, help='GPU ID (CPU 模式使用 0)')
    parser.add_argument('--model', type=str, default=None,
                       help='模型路径或名称 (覆盖 common_stuff.py 中的配置)')
    ...
    if not os.path.exists(data_dir):
        ...
        print('请确保 arc-prize-2024 目录存在并包含数据文件')
        return 1

    # 检查 HuggingFace token（如果需要访问私有模型）
    hf_token = os.environ.get('HF_TOKEN') or os.environ.get('HUGGINGFACE_TOKEN')
    if not hf_token:
        print('\n*** 提示: 如果模型需要认证，请设置环境变量:')
    ...
    try:
        if args.mode in ['train', 'full']:
            ...
            print('开始训练...')
            print('=' * 60)
            start_training(gpu=args.gpu if env_info['cuda_available'] else None)
            print('*** 训练完成')
        ...
        if args.mode in ['inference', 'full']:
            ...
            print('开始推理...')
            print('=' * 60)
            start_inference(gpu=args.gpu if env_info['cuda_available'] else None)
            print('*** 推理完成')
        ...
        if args.mode == 'full':
            ...
            # 检查推理输出是否存在
            if not os.path.exists(infer_params['store']):
                print(f'*** 错误: 推理输出目录不存在: {infer_params["store"]}')
                print('*** 请确保推理阶段已成功完成')
                return 1
            
            # 创建 formatter（如果推理阶段没有创建）
            # 需要从模型加载 formatter
            storage_path = f'{model_temp_storage}_gpu0'
            try:
                if os.path.exists(storage_path):
                    _, formatter = prepare_run(storage_path, train=False, gpu=None if not HAS_CUDA else 0)
                else:
                    # 如果模型不存在，使用基础模型创建 formatter
                    _, formatter = prepare_run(base_model, train=False, gpu=None if not HAS_CUDA else 0)
            except Exception as e:
                print(f'*** 错误: 无法创建 formatter: {e}')
                import traceback
                traceback.print_exc()
                return 1
            
            decoder = Decoder(formatter, arc_test_set.split_multi_replies(), n_guesses=2, prob_baseline=0.05)
            decoder = decoder.from_store(infer_params['store'])
            if use_aug_score or arc_test_set.is_fake:
                decoder.calc_augmented_scores(model=None, store=score_temp_storage, **aug_score_params)
            submission = arc_test_set.get_submission(decoder.run_selection_algo(submission_select_algo))
            with open('submission.json', 'w') as f:
                json.dump(submission, f, indent=2)
            print('*** 提交文件已生成: submission.json')
